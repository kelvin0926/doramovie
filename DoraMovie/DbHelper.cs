...
namespace DataBase
{
    ...
    public abstract class Provider_Class : Page
    {
    ...
    public abstract class DbHelper : Provider_Class
    {
        ...
        //==============================================================================
        public static string q(string _param)
        {
            Regex regex = new Regex("/" + _param + "-(?<class>.*)[.]html");
            _param = string.IsNullOrEmpty(HttpContext.Current.Request.QueryString[_param]) ? "" : HttpContext.Current.Request.QueryString[_param];

            Match match = regex.Match(HttpContext.Current.Request.CurrentExecutionFilePath);
            if (match.Success)
            {
                _param = match.Groups["class"].Value;
            }
            return _param;
        }
        ...
        #region 通过ajax输出结果
        /// <summary>
        /// toast通过ajax输出结果到页面，但注意用try catch时会报错，因为 End() 不能在try里面。
        /// </summary>
        /// <param name="message">通过ajax输出结果到页面</param>
        ...
        public static void JsWindows(string message)
        {
            message = message.Replace("'", "\"");
            HttpContext.Current.Response.Write("<script>alert('" + message + "');history.go(-1)</script>");
